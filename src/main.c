...
#define ZIGBEE_BOOT_OTA_FADDR   0x010000
...
static const uint32_t flag_addr_ok = 0x33CC55AA;
 ...
 * @fn      tuya_zigbee_ota
 *
 * @brief   Performs Zigbee OTA reformat from low OTA to big OTA layout
 *
 * @param   None
 *
 * @return  None
...
_attribute_ram_code_ void tuya_zigbee_ota(void) {
    ...
    uint32_t size;
    uint32_t faddrr = ZIGBEE_BOOT_OTA_FADDR; // 0x008000
    uint32_t faddrw = BIG_OTA1_FADDR;    // 0x000000
    uint32_t buf_blk[64];    // max 256 bytes
 ...
 * @fn      flash_main
 *
 * @brief   Initializes system, OS, user app, and runs the main loop
 *
 * @param   None
 *
 ...
 * @fn      main
 *
 * @brief   Entry point, checks OTA and calls flash_main
 *
 * @param   None
 *
 * @return  int - returns flash_main result
 */
_attribute_ram_code_ int main(void) {

    if(*(uint32_t *)(ZIGBEE_BOOT_OTA_FADDR + 8) == ID_BOOTABLE || flag_addr_ok != 0x33CC55AA) {
